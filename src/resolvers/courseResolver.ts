...
const isOwner = (currentUserId: string, resourceUserId: string): boolean => currentUserId === resourceUserId;
...
export const courseResolver = {
  Query: {
    ...
    course: async (_: unknown, { id }: QueryCourseArgs): Promise<Course | null> => {
      const course = await prisma.course.findUnique({ where: { id }, include: { createdBy: true, collection: true } });
  ...
  Mutation: {
    ...
    updateCourse: async (_: unknown, { id, input }: MutationUpdateCourseArgs, context: GraphQLContext): Promise<Course> => {
      ...
      if (!isAdmin(user.role) && !isOwner(user.id, course.createdById)) {
        throw new GraphQLError("You do not have permission to update this course", {
    ...
    deleteCourse: async (_: unknown, { id }: MutationDeleteCourseArgs, context: GraphQLContext): Promise<string> => {
      ...
      if (!isAdmin(user.role) && !isOwner(user.id, course.createdById)) {
        throw new GraphQLError("You do not have permission to delete this course", {
          extensions: { code: "UNAUTHORISED" },
        });
      }
